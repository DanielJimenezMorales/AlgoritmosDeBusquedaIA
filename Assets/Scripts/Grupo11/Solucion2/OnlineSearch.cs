...
public class OnlineSearch : MonoBehaviour
{
    ...
    public Nodo SearchOnline(BoardInfo board, CellInfo start, CellInfo[] goals)
    {
        ...
        if (isEnemyArrayEmpty)
        {
            ...
            if (nodoStart.getInfo().RowId == goals[0].RowId && nodoStart.getInfo().ColumnId == goals[0].ColumnId)
            {
                return nodoStart;
            }
        }
        else
        {
            ...
            if (nodoStart.getInfo().RowId == currentEnemy.CurrentPosition().RowId
                && nodoStart.getInfo().ColumnId == currentEnemy.CurrentPosition().ColumnId)
            {
                return nodoStart;
            }
        }
        ...
        CellInfo[] hijos = nodoStart.getInfo().WalkableNeighbours(board);
        ...
        for (int i = 0; i < hijos.Length; i++)
        {
            if (hijos[i] != null)
            {
                ...
                Nodo hijoNodo = new Nodo(nodoStart, h, hijos[i]);
                open.Add(hijoNodo);
            }
        }
